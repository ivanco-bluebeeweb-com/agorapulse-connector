# Agorapulse Connector — Auth & Credentials Standard

**App ID:** `agorapulse-connector`  
...
- **Верификация подключения:** При сохранении ключа выполняется тестовый запрос `GET /v1/accounts`.
- **Отключение:** Удаление локальных ключей по команде пользователя без повреждения данных на стороне Agorapulse.
