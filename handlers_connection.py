"""Connection lifecycle for Agorapulse Connector."""
...
_SECRET = "agorapulse_connections"
...
@chat.function(
    "connect_agorapulse",
    "Connect Agorapulse account via credentials.",
    action_type="write",
    chain_callable=True,
...
async def connect_agorapulse(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Agorapulse Connector."""
    client = AgorapulseClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Agorapulse Account",
...
@chat.function(
    "list_connections",
    "List connected Agorapulse accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
async def disconnect_agorapulse(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
